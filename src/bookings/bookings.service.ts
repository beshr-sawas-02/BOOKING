...
import { MahramValidator, calcAge, Gender } from './validators/mahram.validator';
...
export class BookingsService {
  ...
  async create(userId: number, dto: CreateBookingDto) {
    ...
    // العمرة لا تشترط محرماً — المرأة تسافر بدون قيود
    ...
    if (pkg.package_type === 'HAJJ') {
      ...
      // تحقق من التركيبات المحظورة كلياً
      if (this.mahramValidator.isForbiddenCombination(
    ...
    // العمرة: لا قيود على المحارم — الجميع يسافر
    ...
    // أرجع تحذيرات إن وجدت مع النتيجة
    return {
      ...booking,
      warnings,
    };
  ...
  // ─────────────────────────────────────────────────────────
  async updateByUser(bookingId: number, userId: number, dto: { trip_end_date?: string; deposit_due_date?: string; final_payment_due_date?: string }) {
    const booking = await this.db().booking.findUnique({
    ...
    if (booking.booking_status !== 'PENDING')
      throw new BadRequestException('لا يمكن تعديل الحجز بعد مراجعته من الأدمن');
